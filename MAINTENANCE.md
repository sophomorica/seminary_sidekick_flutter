...
| MAINT-009 | Sentry FLUTTER-7 (`FunctionException` status 403 premium gate): TASK-067's `recordError` on the expected `sidekick-proxy` entitlement 403 opened a false crash issue. Stop recording that path (breadcrumb only, same pattern as FLUTTER-6) and drop matching entitlement-gate `FunctionException` events in `beforeSend`. UI Refresh/upgrade handling unchanged. | 2026-07-17 |
